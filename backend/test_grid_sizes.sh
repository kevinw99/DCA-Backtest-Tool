...
# Test to show grid sizes being used for validation

echo "Testing Grid Sizes for Consecutive Buys..."
echo "==========================================="
...
curl -X POST http://localhost:3001/api/backtest/dca \
  ...
  }' 2>/dev/null | jq -r '.data.transactionLog[]' | grep -E "CONSEC BUY COUNT|ACTION: (BUY|TRAILING STOP BUY EXECUTED)" | head -20
