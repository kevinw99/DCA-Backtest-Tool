...
# Test with full logs showing spacing calculations

echo "Testing Spacing Details..."
echo "=========================="
...
curl -X POST http://localhost:3001/api/backtest/dca \
  ...
  }' 2>/dev/null | jq -r '.data.transactionLog[]' | grep -E "Lot #|spacing=|required=|DEBUG: Before spacing" | head -30
