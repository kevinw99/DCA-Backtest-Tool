...
# Test to show consecutive buy counts and spacing validation

echo "Testing Consecutive Buy Count Tracking..."
echo "=========================================="
...
curl -X POST http://localhost:3001/api/backtest/dca \
  ...
  }' 2>/dev/null | jq -r '.data.transactionLog[]' | grep -E "CONSEC BUY COUNT|ACTION: BUY|ACTION: SELL|RESET.*Consecutive"
